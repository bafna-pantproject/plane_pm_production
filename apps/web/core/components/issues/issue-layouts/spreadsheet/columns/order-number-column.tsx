...
export const SpreadsheetOrderNumberColumn = observer(function SpreadsheetOrderNumberColumn(props: Props) {
  const { issue } = props;

  return (
    <div className="flex h-11 items-center border-b-[0.5px] border-subtle px-page-x">
      <span className="truncate text-13">{issue.order_number}</span>
    </div>
  );
});
